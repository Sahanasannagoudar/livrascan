document.addEventListener('DOMContentLoaded', () => {
    ...
    // Reading animation for hero section
    const readingVal = document.querySelector('.reading');
    if (readingVal) {
        let count = 0;
        const target = 11.2;
        const interval = setInterval(() => {
            if (count >= target) {
                ...
                readingVal.innerHTML = `${target.toFixed(1)} <span>mg/dL</span>`;
            } else {
                count += 0.3;
                readingVal.innerHTML = `${count.toFixed(1)} <span>mg/dL</span>`;
            }
        }, 50);
    }
});
